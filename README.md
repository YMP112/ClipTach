...
- פתיחת תמונה
- מסך פתיחה עם פתיחת תמונה/טעינת פרויקט ורשימת פרויקטים אחרונים
- סימון מסכה ידני (`Keep` / `Erase`)
- כלי `Polygon Keep` לסימון בקווים ישרים ושמירה של כל מה שבתוך הצורה
- בניית Alpha Mask
- Auto Assist משופר להצעת אובייקט התחלתית
- תצוגת אובייקט מחולץ על רקע שקוף
- טרנספורמציות בסיסיות: `Move`, `Scale`, `Rotate`, `Skew`
- עריכת אובייקט עם שליטה מספרית (`Scale px`, `Rotate°`, `Skew°`) ואיפוס נפרד לכל ערך
- ייצוא PNG שקוף
- ייצוא עם דיאלוג אפשרויות: `With margins (px)` או `Object only (tight rectangle)` עם זכירת הבחירה האחרונה
- שם ייצוא ברירת מחדל: `<original>_clear.png`
